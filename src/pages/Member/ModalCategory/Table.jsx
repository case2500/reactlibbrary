...
export const Table = ({ rows, editRow, delCategory }) => {
  ...
  return (
    <div className="table-wrapper  ">
      <div className="table-wrapper  min-h-[120px]">
        <table className="px-1 py-2 text-sm text-left text-gray-500 bg-blue-200 w-1/8">
          <thead className="text-xs text-gray-700 uppercase ">
            <tr className="w-32  text-xl ">
              <th className="w-32 px-2 ">No</th>
          ...
          <tbody>
            {currentItems?.map((row, idx) => (
              ...
              >
                <td className="w-16 mx-auto px-5  text-xl ">
                  {" "}
                  {idx + itemOffset + 1}.
                </td>
                <td className="w-64 mx-auto text-xl ">{row.name}</td>

                <td className="w-64 mx-auto text-xl ">
                {row?.image ? (
                     <>
