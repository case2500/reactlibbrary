...
import { getMembers } from "../../redux/features/member/memberSlice.jsx";
import { getProducts } from "../../redux/features/product/productSlice";

const Dashboard = () => {
  const API_URL = `${URL}/dashboard`;
  const API_URLStatus = `${URL}/checkbook`;
  ...
  const getShowBookBorrow = async () => {
    ...
    //  alert(JSON.stringify(response.data.reverse()))
  };
  useEffect(() => {
    // setData([])
    getShowBookBorrow();
  ...
  const getShowStatus = async () => {
    const response = await axios.get(API_URLStatus);
    setShowStatus(response.data);
  ...
  return (
    <div>
      ...
      <div className="rounded-sm px-10  border border-stroke bg-white py-6 px-7.5 shadow-default dark:border-strokedark dark:bg-boxdark">
        {data.map((p, index) => (
          <>{p.username}</>
        ))}

        <div className=" flex items-end justify-between ">
          <div className="w-[250px] h-[100px] rounded-md py-5 text-center text-white text-xl font-bold  dark:text-white bg-green-500">
            จำนวนรายการหนังสือ
            <p>{products.length} รายการ </p>
          </div>
          <div className="w-[250px] h-[100px] rounded-md py-5 text-center text-white text-xl font-bold  dark:text-white bg-red-600">
            จำนวนรายการหนังสือที่ยืม
            <p>{showstatus.length} รายการ </p>
          </div>
          <div className="w-[250px] h-[100px] rounded-md py-5 text-center text-white text-xl font-bold  dark:text-white bg-purple-700">
            จำนวนสมาชิก
            <p>{members.length} คน </p>
          </div>
        </div>
      </div>
      <div className="h-[450px] ">
        <div>จำนวนหนังสือที่ทำรายการยืม </div>
        <ResponsiveContainer>
          <LineChart
            width={500}
            height={300}
          ...
          >
            ...
            <Line
              ...
              activeDot={{ r: 8 }}
            />
            <Line type="monotone" dataKey="amout" stroke="#82ca9d" />
          </LineChart>
        </ResponsiveContainer>
      </div>
